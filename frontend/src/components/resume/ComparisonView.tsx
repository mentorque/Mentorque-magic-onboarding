    ...
    >
      <div className={cn("flex items-center justify-between shrink-0", headerClassName)}>
        <div className="flex items-center gap-3">
          ...
          <h4 className="text-[10px] font-black uppercase tracking-[0.25em] text-white/60 group-hover/card:text-white/90 transition-colors">{title}</h4>
        </div>
        {rightElement}
      </div>
    ...
    >
      <div className="h-full flex flex-col gap-5">
        <AnimatePresence mode="wait" custom={dir}>
          ...
          >
            <div className="relative pl-6">
              <div className="absolute left-0 top-1 bottom-1 w-[2px] bg-red-500/40 rounded-full" />
              <p className="text-[11px] font-bold uppercase tracking-widest text-red-400/60 mb-1.5">Original</p>
              <p
                className="text-sm leading-relaxed font-normal text-white/50"
                style={{ textDecoration: 'line-through', textDecorationColor: 'rgba(248,113,113,0.7)', textDecorationThickness: '2px' }}
              >
                {visible?.original}
              </p>
            </div>
            <div className="relative pl-6">
              ...
              <p className="text-[11px] font-bold uppercase tracking-widest text-emerald-400/80 mb-1.5">Optimized</p>
        ...
        <div className="mt-auto flex items-center justify-between border-t border-white/5 pt-4">
          <div className="flex items-center gap-3">
            <span className="text-[10px] font-mono font-bold text-white/70">{String(idx + 1).padStart(2, '0')}</span>
            <div className="w-8 h-px bg-white/20" />
            <span className="text-[10px] font-mono text-white/40">{String(changes.length).padStart(2, '0')}</span>
          </div>
              ...
              <div className="grid grid-cols-4 sm:grid-cols-8 gap-4">
                {COMPANIES.map((c) => (
                  ...
                  >
                    ...
                    <span className="text-[9px] font-bold text-white/70 group-hover/logo:text-white transition-colors uppercase tracking-widest">{c.name}</span>
                  </button>
                ))}
              </div>
                ...
                {(() => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="text-[10px] font-black uppercase tracking-widest text-white/85">{selectedCompany}</span>
                    </motion.div>
                  );
                })()}
              </div>
...
function MetricItem({ m }: { m: any }) {
  return (
    <div className="group/metric relative flex flex-col justify-between p-4 rounded-3xl bg-white/[0.04] border border-white/10 hover:bg-white/[0.07] hover:border-white/20 transition-all duration-500 shadow-xl overflow-hidden min-h-[120px]">
      ...
      <div>
        <p className="text-[10px] font-black uppercase tracking-[0.2em] text-white/60 mb-1 leading-none">{m.label}</p>
        <div className="flex items-baseline gap-1.5">
          <span className="text-2xl font-mono font-bold text-white leading-none">{m.value}</span>
          <span className="text-sm font-mono font-bold text-white/60 leading-none">{m.suffix}</span>
        </div>
      </div>
    </div>
  );
...
function MetricsCard({ revampedResume, originalResume }: { revampedResume: any; originalResume?: any }) {
  ...
  const items = [
    ...
    { label: 'Strength', value: curr.strength, prev: orig.strength, suffix: '%' },
      ...
      <div className="flex items-center justify-between px-6 mt-4 shrink-0">
        <div className="flex items-center gap-5">
          ...
          <div className="flex flex-col gap-0.5">
            ...
            <p className="text-[10px] text-white/60 font-black uppercase tracking-widest">Section Analysis</p>
          </div>
        </div>
...
export function ComparisonView({
  ...
  useEffect(() => {
    ...
    const fetchStudioComments = async () => {
      try {
        const res = await fetch(`/api/highlights?documentUrl=${encodeURIComponent(documentId)}`);
        const data = await res.json();
        if (!data?.success || !Array.isArray(data.highlights)) return;

        const flattened: StudioCommentItem[] = data.highlights.flatMap((h: any) => {
          ...
          return comments.map((c: any) => ({
            ...
            text: String(c?.text ?? ''),
            selectedText: typeof selectedText === 'string' ? selectedText : undefined,
            createdAt: String(c?.createdAt ?? h?.createdAt ?? new Date(0).toISOString()),
          }));
        });
          ...
          {activeTab === 'analysis' ? (
            ...
            <div className="relative rounded-[2rem] border border-blue-400/20 bg-blue-950/20 p-6 overflow-hidden">
              ...
              <div className="relative z-10">
                <h2 className="text-2xl font-bold tracking-tight text-white mb-1">Resume Studio</h2>
                <p className="text-white/60 text-sm mb-5">All AI revamps and note comments from your PDF annotations.</p>

                <div className="space-y-3 max-h-[65vh] overflow-y-auto pr-1 custom-scrollbar">
                  {studioComments.length === 0 ? (
                    <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4 text-sm text-white/60">
                      No comments yet. Select text in the PDF and use <span className="text-white/90 font-semibold">Ask AI</span> or <span className="text-white/90 font-semibold">Note</span>.
                    </div>
                  ) : (
